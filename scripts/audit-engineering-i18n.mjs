...
const root = "src/Rools/engineering/pages";
...
const add = (value, file) => {
  ...
  if (text.startsWith("Tiles © Esri")) return;
...
if (found.size) {
  console.error(`Missing ${found.size} engineering translations:\n`);
  [...found].sort(([a], [b]) => a.localeCompare(b)).forEach(([text, sources]) => console.error(`- ${JSON.stringify(text)} (${[...sources].join(", ")})`));
  process.exitCode = 1;
} else console.log("Engineering UI translation audit passed.");
